...
docker exec apps-postgres psql -U postgres -d apps_db -c "SQL"  # 查询数据库
```
...
- `apps-server` — Rust 二进制（内嵌前端静态文件 + Prisma migration）
...
- 如果确实必须运行涉及数据库的操作，**必须先备份**：`docker exec apps-postgres pg_dump -U postgres -d apps_db > .data/backup_pre_test.sql`
