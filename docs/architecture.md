...
├── apps-postgres (postgres:18) — 仅 Docker 内网
└── apps-server (apps-server:latest) — 端口映射到 WEB_PORT（默认 8080）
    ├── Rust 二进制 (/usr/local/bin/apps-server)
    ├── 前端静态文件 (/app/web)
...
make dev
  ...
  │     ├── ts-rs 类型导出（后台）
  │     └── apps-server --listen 0.0.0.0:5678
